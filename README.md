...
- **ollama** — local, via the bundled `ollama` container. Non-repository workflows may point `OLLAMA_BASE_URL` at a bigger box on your network (see below); repository analysis requires the bundled service or a loopback endpoint in v1. Default model: `qwen3:8b`.
...
media and ordinary library workflows. Ollama-assigned non-repository steps then
run there with no code changes; point Settings at bigger model names once they
are pulled on that box. Repository analysis intentionally rejects LAN/remote
Ollama endpoints in v1 and requires the bundled service or a loopback endpoint.
