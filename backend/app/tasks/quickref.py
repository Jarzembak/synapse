"""Quick-reference docs: one per tool / technique, auto-merged across videos.
...
def quickref(job_id: int, project_id: int):
    ...
    progress(job_id, "identifying tools and techniques")
    extraction = llm.complete_json(
        ...
        + json.dumps(index)
        + '\n\nReply as {"entities": [{"name": "...", "kind": "tool|technique", '
    ...
    for i, ent in enumerate(entities):
        ...
        kind = ent.get("kind")
        if not name or kind not in ("tool", "technique"):
            continue
...
def _upsert_quickref(project_id: int, project_slug: str, project_title: str,
    ...
    if ref:
        ...
        body = llm.complete(
            "quickref",
            prompts.QUICKREF_MERGE_SYSTEM + source_note,
            f"EXISTING DOCUMENT:\n\n{existing_body}\n\n---\n\n"
        ...
        body = llm.complete(
            "quickref",
            prompts.QUICKREF_NEW_SYSTEM.format(kind=kind) + source_note,
            f"Subject: {name}\n\n{deepdive[:80000]}",
        ).strip()
